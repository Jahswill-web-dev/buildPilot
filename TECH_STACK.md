...
### Laravel Queues
Idea roadmap generation runs through a queued job so browser requests can return immediately. The project uses the database queue driver by default and requires a queue worker such as `php artisan queue:work` or the `composer dev` script's queue listener.
...
The `Idea` model stores an idea name, description, status, owner, generated roadmap sections, JSON action phases, JSON checklist, and JSON Action Plan tasks. JSON roadmap, phase, checklist, and task columns are cast to PHP arrays in `app/Models/Idea.php`.
...
- AI-generated roadmap phase storage via the `action_phases` JSON column.
...
### LangChain.js + OpenAI
New idea roadmaps are generated by a queued Laravel job that calls the Node bridge at `resources/js/ai/generate-roadmap.mjs`. It uses `@langchain/openai`, `@langchain/core`, and `zod` to request structured roadmap data from OpenAI. Prompt text lives in `resources/prompts/roadmap/`, including dedicated prompts for profile, core features, MVP scope, phases, and checklist generation.
...
| `AI_ROADMAP_TIMEOUT_SECONDS` | `120` | Timeout for the server-side LangChain bridge process |
